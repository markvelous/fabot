...
async function liquidity(amount0,amount1,amount2,amount3,amount4) {
    ...
    gasLimit = await token0.deploy({arguments: ['GovTech', 'RJ']}).estimateGas()
    receipt = await token0.deploy({arguments: ['GovTech', 'RJ']}).send({from: myAccount,gas: gasLimit})
    token0.options.address = receipt._address

    //deploying token1
    gasLimit = await token1.deploy({arguments: ['Remarkable', 'RMK']}).estimateGas()
    receipt = await token1.deploy({arguments: ['Remarkable', 'RMK']}).send({from: myAccount,gas: gasLimit})
